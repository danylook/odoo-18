...
class AccountDebtLine(models.Model):
    ...
    _auto = False
    _order = 'date asc, date_maturity asc, id'
    _depends = {
        'res.partner': [
            'user_id',
        ],
    ...
    def _compute_move_lines_data(self):
        ...
        for rec in self:
            ...
            rec.financial_amount = sum(
                rec.move_line_ids.mapped('amount_currency'))
            rec.financial_amount_residual = sum(
                rec.move_line_ids.mapped('amount_residual_currency'))
    ...
    def init(self):
        ...
        query = """
            SELECT
                ...
                --     a.user_type_id, am.document_type_id,
    ...
    def cancel_amount_residual_currency(self):
        ...
        for aml in self.mapped('move_line_ids'):
            ...
            exchange_move = self.env['account.move'].create(
                self.env['account.full.reconcile'].process_trade_difference(
