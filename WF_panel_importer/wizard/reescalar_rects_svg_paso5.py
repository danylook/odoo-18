...
INPUT_SVG = "C:\\odoo17\\extra-addons\\others-17.0\\easyOCR\\svg_pages\\L1_E14_paso4_coloreado.svg"
OUTPUT_SVG = "C:\\odoo17\\extra-addons\\others-17.0\\easyOCR\\svg_pages\\reescalado_elevaciones_de_paredes_rects8.svg"
...
def alinear_rects_superior_y_frontal_por_color(svg, color):
    ...
    if not grupo:
        ...
        return
    # Use the frontal rect's x as anchor — not min_x of the group — because the
    # superior rect may span a wider area (e.g. full header zone) whose left edge
    # is to the left of the actual piece, pulling all rects to the wrong position.
    ref_x = float(rect_frontal.getAttribute('x'))
    max_x = ref_x + float(rect_frontal.getAttribute('width'))
    ancho_total = max_x - ref_x
    # Mover todos al x del frontal
    for r in grupo:
        r.setAttribute('x', str(ref_x))
