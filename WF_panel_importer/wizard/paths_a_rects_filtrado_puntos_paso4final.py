...
PROPORCION_HORIZONTAL = 0.2969  # FALLBACK ONLY — overridden at runtime by calcular_proporcion_por_moda_rects
PROPORCION_VERTICAL = 0.2969    # FALLBACK ONLY — overridden at runtime by calcular_proporcion_por_moda_rects
MARGEN_LARGO = 0.25  # 1/4 de pulgada
MARGEN_ANCHO = 0.25  # 1/4 de pulgada


INPUT_SVG = "C:\\odoo17\\extra-addons\\others-17.0\\easyOCR\\svg_pages\\L1_E18_paso3_absoluto.svg"

#INPUT_SVG = "C:\odoo17\extra-addons\others-17.0\easyOCR\svg_pages\elevaciones de paredes_paso3_absoluto_page8.svg"
OUTPUT_SVG = "C:\odoo17\extra-addons\others-17.0\easyOCR\svg_pages\elevaciones_de_paredes_rects_filtrado_puntos8.svg"
...
def calcular_proporcion_por_moda_rects(svg, piezas):
    """
    Auto-detecta la escala px/pulgada del SVG usando la dimensión mínima de los <rect>.

    La dimensión más pequeña en cualquier panel de entramado de madera es siempre
    1.5" (grosor de placa de 2x). Dividiendo el mínimo en píxeles por 1.5 obtenemos
    la escala real de este SVG, independientemente del zoom con que fue dibujado.

    # ── ROLLBACK: lógica anterior (moda de studs + pieza de referencia del BOM) ──────
    # global PROPORCION_HORIZONTAL, PROPORCION_VERTICAL
    # rects_intermedios = [el for el in svg.getElementsByTagName('*') if el.tagName.lower().endswith('rect')]
    # lados = []
    # for rect in rects_intermedios:
    ...
    for rect in rects_intermedios:
        try:
            ...
            continue
        if w > 0.5 and h > 0.5:
            narrow_dims.append(round(min(w, h), 6))  # dimensión estrecha de cada pieza
    if not narrow_dims:
        print("[PROPORCION] No se encontraron <rect> con dimensiones válidas. Se conserva PROPORCION por defecto.")
        return None
    # Usar la MODA de las dimensiones estrechas (min(w,h) por rect).
    # Esto es robusto: un elemento pequeño aislado (OSB edge, nailer) no corrompe el resultado.
    # En todo panel de entramado, la dimensión estrecha dominante es siempre 1.5" (grosor de placa de 2x).
    moda_px, count = Counter(narrow_dims).most_common(1)[0]
    proporcion = moda_px / 1.5  # 1.5" = grosor real de una placa de 2x
    PROPORCION_HORIZONTAL = proporcion
    PROPORCION_VERTICAL = proporcion
    print(f"[PROPORCION] Auto-detectada: moda_narrow={moda_px:.4f}px (×{count}) / 1.5\" = {proporcion:.6f} px/pulgada")
    return proporcion
...
def main():    
    ...
    # Build a lookup: data-largo (inches str) -> (x, y) from already-labeled sheathing rects
    sheathing_pos_by_largo = {}
    for rect in svg.getElementsByTagName('*'):
        if not rect.tagName.lower().endswith('rect'):
            continue
        rid = rect.getAttribute('id') if rect.hasAttribute('id') else ''
        if 'sheathing' not in rid.lower():
            continue
        dl = rect.getAttribute('data-largo') if rect.hasAttribute('data-largo') else ''
        if not dl:
            continue
        if dl not in sheathing_pos_by_largo:
            try:
                sx = float(rect.getAttribute('x') or 0)
                sy = float(rect.getAttribute('y') or 0)
                sheathing_pos_by_largo[dl] = (sx, sy)
            except ValueError:
                pass
    ...
    for etiqueta, descripcion, cantidad, medida, width_str in SHEATING:
        ...
        label_normalizado = normalizar_id_svg(descripcion)
        new_id = f"{etiqueta}_{label_normalizado}"
        # width=ancho (horizontal), height=largo (vertical) — el sheathing es vertical
        ...
        # Position: use x,y of already-labeled sheathing with same largo, else fall back to panel origin
        largo_key = f"{largo_real:.3f}"
        if largo_key in sheathing_pos_by_largo:
            pos_x, pos_y = sheathing_pos_by_largo[largo_key]
        else:
            pos_x, pos_y = panel_x_min, panel_y_min
        rect_s = doc.createElement('rect')
        rect_s.setAttribute('id', new_id)
        rect_s.setAttribute('data-id', new_id)
        rect_s.setAttribute('x', str(round(pos_x, 4)))
        rect_s.setAttribute('y', str(round(pos_y, 4)))
        ...
        rects.append(rect_s)
        print(f"[SHEATHING-SINTETICO] {descripcion} -> {new_id} x={pos_x:.1f} y={pos_y:.1f} w={rect_w:.1f} h={rect_h:.1f} largo={largo_real:.3f}\" ancho={ancho_real:.3f}\"")
