...
from odoo.addons.payment_mercado_pago_uruguay.tests.common import MercadoPagoCommon
...
class TestPaymentTransaction(MercadoPagoCommon, PaymentHttpCommon):
    ...
    def test_no_item_missing_from_preference_request_payload(self):
        ...
        request_payload = tx._mercado_pago_uy_prepare_preference_request_payload()
        self.maxDiff = 10000  # Allow comparing large dicts.
        return_url = 'https://prueba.ecoguard.com.uy/get?back_url=/success'  # Ajusta según tu lógica
        webhook_url = 'https://prueba.ecoguard.com.uy/webhook/Test%20Transaction'
        sanitized_reference = url_quote(tx.reference)
        self.assertDictEqual(request_payload, {
            'auto_return': 'approved',
            'back_urls': {
                'success': 'https://prueba.ecoguard.com.uy/get?back_url=/success',
                'pending': 'https://prueba.ecoguard.com.uy/get?back_url=/pending',
                'failure': 'https://prueba.ecoguard.com.uy/get?back_url=/failure',
            },
            ...
            'notification_url': f'https://prueba.ecoguard.com.uy/webhook/{sanitized_reference}',
            'payer': {
    ...
    def test_no_input_missing_from_redirect_form(self):
        ...
        tx = self._create_transaction(flow='redirect')
        with patch(
            'odoo.addons.payment_mercado_pago_uruguay.models.payment_transaction.PaymentTransaction'
    ...
    def test_processing_notification_data_confirms_transaction(self):
        ...
        tx = self._create_transaction(flow='redirect')
        with patch(
            'odoo.addons.payment_mercado_pago_uruguay.models.payment_provider.PaymentProvider._mercado_pago_uy_make_request',
            return_value=self.verification_data
        ):
            tx._process_notification_data(self.redirect_notification_data)
        self.assertEqual(tx.state, 'done')

    @mute_logger('odoo.addons.payment_mercado_pago_uruguay.models.payment_transaction')
    def test_processing_notification_data_rejects_transaction(self):
        ...
        with patch(
            'odoo.addons.payment_mercado_pago_uruguay.models.payment_provider.PaymentProvider._mercado_pago_uy_make_request',
            return_value=self.verification_data_for_error_state
        ):
            tx._process_notification_data(self.redirect_notification_data)
